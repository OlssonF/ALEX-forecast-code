...
# Script to model a scenario that is just the entitlement flow
# this is based on a monthly minimum entitlement flow at the SA border (can we get estimated flow at Wellington?)
# then applying the same "process" model of losses and travel time

#' Title
#'
#' @param config 
#' @param entitlement_dat location of the entitlement data from DEW (monthly in ML/d)
#'
#' @returns
#' @export
#'
#' @examples
generate_entflow_inflow_fc <- function(config,
                                       entitlement_dat = 'R/helper_data/entitlement_flow.csv',
                                       lag_t = 14) {
  # read in data
  ent_flow <- read_csv(entitlement_dat, col_names = c('month', 'ent_MLd'),
  ...
  upstream_start <- reference_date - days(lag_t)
  
  
  forecast_dates <- seq.Date(as_date(upstream_start), length.out = horizon + lag_t, by = 1)
  
  message("fixed daily flow by month")
  
  ent_fc <- tibble(datetime = forecast_dates,
                   month = month(datetime, label = T, abbr = F)) |> 
    left_join(ent_flow, by = join_by(month)) |> 
    mutate(reference_datetime = reference_date,
           site_id = site_id) |> 
    select(reference_datetime, datetime, ent_MLd,  site_id)
  message("generating flow forecast at SA border - apply loss model for Wellington")
  message("returning flow in ML/day - convert before using in FLARE!")
  return(ent_fc) # flow in ML/day
}

#' Title
#'
#' @param config 
#' @param eflow_dat location of the entitlement data from DEW (monthly in GL/m)
#'
#' @returns
#' @export
#'
#' @examples
generate_eflow_inflow_fc <- function(config,
                                     eflow_dat = 'R/helper_data/eflow.csv',
                                     lag_t = 14) {
  # read in data
  eflow <- read_csv(eflow_dat, show_col_types = F) 
  ...
  eflow_fc <- tibble(datetime = forecast_dates,
                     month = month(datetime, label = T, abbr = F)) |> 
    left_join(eflow, by = join_by(month)) |> 
    mutate(days_in_month = lubridate::days_in_month(datetime),
           eflow_MLd = (eflow_GLm/days_in_month) * 1000, # convert to ML/d - same units as the entitlement flow
           reference_datetime = reference_date,
           site_id = site_id) |> 
    select(reference_datetime, datetime, eflow_MLd,  site_id)
...
#----- Functions to convert QSA to Wellington using loss model----
...
generate_flow_inflow_fc <- function(config,
  ...
  if (!upstream_location %in% c('L1', 'QSA')) {
    ...
    upstream_MLd <- read_csv('data_raw/upstream.csv', show_col_types = F, skip = 5,
