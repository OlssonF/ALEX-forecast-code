...
generate_salt_inflow_fc <- function(config,
  ...
  met_combined <- bind_rows(met_s3_historical, met_s3_future) |> 
    ...
    group_by(parameter) |> 
    mutate(threeday_precip = RcppRoll::roll_sum(precip, n = 3, fill = NA,align = "right"), # calculate a 15-day rolling sum
  ...
  salt_training_df <- training_df |> 
    filter(variable == "SALT", inflow_name == 'murray') |> 
    select(-precip) |> 
    # add in a lagged predictor of lag(obs)
    mutate(lag_obs = lag(observation, 1)) |> 
    select(any_of(c('date', 'precip', 'temperature', 'threeday_precip', 'threeday_temp', 'doy', 'observation', 'lag_obs')))
  ...
  for (i in 1:length(forecast_dates)) {
    ...
    previous_prediction <- salt_forecast |> filter(date == forecast_dates[i] - days(1)) |> pull(prediction)
    ...
    salt_forecast <- bind_rows(salt_forecast, ens_predictions)
